...
void publishMarkers(visualization_msgs::MarkerArray &markers) {
    ...
    if (g_collision_points.markers.size()) {
        for (int i = 0; i < g_collision_points.markers.size(); i++)
            g_collision_points.markers[i].action =
...
std::vector<cfc::SuperQuadrics> constructObstacleSQModel(
...
void computeCollisionContactPoints(InteractiveRobot &robot) {
    ...
    checkCollision(
        links, obstacle, robot.robotState(), &c_req, &c_res,
...
void queryMinimumDistance(InteractiveRobot &robot) {
    ...
    queryRobotWorldDistance(links, obstacle, robot.robotState(), &d_req,
                            &d_res);
